...
class BasePage(object):
    ...
    def get_windows_img(self):
        """
        在这里我们把file_path这个参数写死，直接保存到我们项目根目录的一个文件夹.\Screenshort下
        """
        time.sleep(1)
        file_path = os.path.dirname(os.path.abspath('.')) + '/screenshort/'
        rq = time.strftime('%Y%m%d%H%M%S', time.localtime(time.time()))
        screen_name = file_path + rq + '.png'
        try:
            self.driver.get_screenshot_as_file(screen_name)
            logger.info("Had take screenshot and save to folder : /screenshort")
        except NameError as e:
            logger.error("Failed to take screenshot! %s" % e)
            self.get_windows_img()

            # 定位元素方法
    def get_windows_img1(self):
        """
        在这里我们把file_path这个参数写死，直接保存到我们项目根目录的一个文件夹.\Screenshort下
        """
        time.sleep(1)
        file_path = os.path.dirname(os.path.abspath('.')) + '/screenshort1/'
        rq = time.strftime('%Y%m%d%H%M%S', time.localtime(time.time()))
        screen_name = file_path + rq + '.png'
        try:
            self.driver.get_screenshot_as_file(screen_name)
            logger.info("Had take screenshot and save to folder : /screenshort1")
        except NameError as e:
            logger.error("Failed to take screenshot! %s" % e)
            self.get_windows_img()
    ...
    def get_windows_img3(self):
        ...
        try:
            ...
            # 定位元素方法
    #移动鼠标
    def move_el(self, selector):

        el = self.find_element(selector)
        try:
            ActionChains(BrowserEngine(self.driver).driver).move_to_element(el).perform()
    ...
    def acclick(self,selector):
        ...
        try:
            ...
            logger.info("The element \' %s \' was acclick." % el.text)
        except NameError as e:
    ...
    def right_click(self, selector):
        ...
        try:
            ...
            # 或者网页标题
    def yidong(self, selector):
        el = self.find_element(selector)
        try:
            ActionChains(BrowserEngine(self.driver).driver).move_to_element_with_offset(el,-20,-15)
            logger.info("The element \' %s \' was yidong_clicked." % el.text)
        except NameError as e:
            logger.error("Failed to click the element with %s" % e)
    ...
    def login(self):
        self.driver.find_element_by_xpath('/html/body/div/div[3]/div/div/div[2]/form/div[1]/div/input[1]').send_keys(
            'admin')
        logger.info('登录账户admin')
        self.driver.find_element_by_xpath('/html/body/div/div[3]/div/div/div[2]/form/div[1]/div/input[2]').send_keys(
            '123456')
        logger.info('登录密码123456')
        self.driver.find_element_by_xpath('/html/body/div/div[3]/div/div/div[2]/div/button').click()
        logger.info("点击登录")
        time.sleep(1)